...
class _CustomTabBarState extends State<CustomTabBar> {
  ...
  Widget build(BuildContext context) {
    return TabBar(
      ...
      controller: widget.tabManager.controller,
      isScrollable: true,
      labelPadding: EdgeInsets.zero,
      indicatorPadding: EdgeInsets.zero,
      padding: EdgeInsets.zero,
      indicator: const BoxDecoration(border: Border(bottom: BorderSide.none)),
      tabs: widget.tabManager.tabs.map((path) {
        return Container(
            padding: const EdgeInsets.only(left: 12.0, top: 2.0),
            decoration: BoxDecoration(
                color: widget.tabManager.controller.index ==
                        widget.tabManager.tabs.indexOf(path)
                    ? Theme.of(context).primaryColor.withOpacity(0.2)
                    : Colors.transparent,
                border: const Border(
                    right: BorderSide(color: Colors.grey, width: 1))),
            child: CustomTab(
              isDirty: widget.tabManager.isTabDirty(path),
