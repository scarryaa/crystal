...
void main() {
  ...
  group('Multi-Selection Tests', () {
    ...
    //  );
    //
    //  final mockSelection = MockSelection();
    //  when(mockSelection.anchor).thenReturn(5);
    //  when(mockSelection.startLine).thenReturn(0);
    //  when(mockSelection.endLine).thenReturn(0);
    //  when(mockSelection.startIndex).thenReturn(5);
    //  when(mockSelection.endIndex).thenReturn(10);
    //  when(mockSelection.originalDirection)
    //      .thenReturn(SelectionDirection.forward);
    //
    //  final selections = [mockSelection];
    //
    //  when(mockSelectionManager.hasSelection()).thenReturn(true);
    //  when(mockSelectionManager.selections).thenReturn(selections);
    //
    //  final firstSelection = selections.first;
    //  printOnFailure(
    //      'mockSel anchor: ${firstSelection.anchor} startLine ${firstSelection.startLine} endLine ${firstSelection.endLine} startIndex ${firstSelection.startIndex} endIndex ${firstSelection.endIndex}');
    //  final cursor = MockCursor()
    //    ..line = mockSelection.endLine
    //    ..index = mockSelection.anchor;
    //
    //  printOnFailure('mockCursor ${cursor.line} ${cursor.index}');
    //
    //  when(mockCursorManager.firstCursor()).thenReturn(cursor);
    //  editorCore.deleteSelectionsIfNeeded();
    //
